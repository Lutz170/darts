...
function submitRound() {
    ...
    throws.forEach(throw_ => {
        if (throw_ === 60) players[currentPlayer].stats.triple20++;
        if (throw_ === 57) players[currentPlayer].stats.triple19++;
        if (throw_ === 54) players[currentPlayer].stats.triple18++;
    });

    if (roundTotal > players[currentPlayer].stats.highest) {
        players[currentPlayer].stats.highest = roundTotal;
    }
    if (roundTotal === 180) {
        players[currentPlayer].stats.count180++;
    }
...
function updateUI() {
    players.forEach((player, index) => {
        ...
        playerElement.querySelector('.count-180').textContent = player.stats.count180;
        playerElement.querySelector('.highest').textContent = player.stats.highest;
        playerElement.querySelector('.t20-count').textContent = player.stats.triple20;
        playerElement.querySelector('.t19-count').textContent = player.stats.triple19;
        playerElement.querySelector('.t18-count').textContent = player.stats.triple18;
    });

    const historyHTML = players[currentPlayer].history.map((round, index) => `
        <div class="entry ${round.roundTotal === 180 ? 'highlight' : ''}">
            Runde ${index + 1}: ${round.throws.join(' + ')} = ${round.roundTotal} 
            (Rest: ${round.remaining})
            ${round.roundTotal === 180 ? '🎯' : ''}
        </div>
    `).join('');
